...
def kfold(ip):
        dataset = pd.read_csv(ip,index_col = 0)
        columns = dataset.columns[:-2]

        #Data Split Train: 80, Test: 20 Vanilla run
        # X =  dataset[columns]
        ...
        for j in range(0,10):
            ...
            X =  train_set[columns]
            Y =  train_set[["FRAUD_NONFRAUD_Fraud","FRAUD_NONFRAUD_Non-Fraud"]]

            X_train, X_test, Y_train, Y_test = train_test_split(X, Y, test_size=0.20, random_state=1, stratify=Y)
            Y_train_svm = Y_train.iloc[:,-1:]
            Y_test_svm = Y_test.iloc[:,-1:]
